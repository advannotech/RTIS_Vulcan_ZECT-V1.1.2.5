...
{
    public partial class ucReOpenSupervisor : UserControl
    {
        ...
        public ucReOpenSupervisor(Panel _parent, frmMain _main)
        {
            InitializeComponent();
            parent = _parent;
            main = _main;
        ...
        private void ucReOpenSupervisor_Load(object sender, EventArgs e)
        {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            try
            {
                string userPerms = Client.GetUserPermissionsReOpen(txtPassword.Text);
                if (userPerms != string.Empty)
                {
                    switch (userPerms.Split('*')[0])
                    {
                        case "1":
                            ...
                            if (found == true)
                            {
                                ucReopenType reOpen = new ucReopenType(parent, main);
